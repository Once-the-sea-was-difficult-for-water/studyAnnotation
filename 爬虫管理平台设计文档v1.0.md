...
> **方案选型说明**：采用「Supervisor XML-RPC 内网暴露 + OSS 分发 conf 文件」方案。Worker 节点无需部署任何业务代码（无 agent.py），仅运行 `supervisord` 和标准 `sshd`；进程控制、状态查询、日志读取全部由平台服务通过 Supervisor XML-RPC 远程完成，conf 文件通过 SSH 直接写入。
...
public class TaskDispatchService {
    ...
    public ExecutionRecord dispatch(Task task, WorkerNode worker) throws Exception {
        ...
        // ② SSH 写入 conf 后 supervisorctl 已同步，program 立即可用，直接 startProcess
