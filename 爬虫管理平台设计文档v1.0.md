...
| `OSS_PARAMS_KEY` | 平台计算：`workers/{nodeCode}/params/{taskCode}.json` | 任务运行参数文件在 OSS 的路径 |
...
```
OSS: workers/{nodeCode}/params/{taskCode}.json
```
...
private String buildConfContent(String execCode, Task task, WorkerNode worker,
    ...
    // 平台计算：workers/{nodeCode}/params/{taskCode}.json
    // 按 taskCode 存储，同一任务多次执行共用，无需重复上传（参数未变时）
    String ossParamsKey = "workers/" + worker.getNodeCode()
                        + "/params/" + task.getTaskCode() + ".json";
...
public ExecutionRecord dispatch(Task task, WorkerNode worker) throws Exception {
    ...
    // 序列化 form_data 并上传 params.json（taskCode 路径，已存在则覆盖）
    String paramsJson   = objectMapper.writeValueAsString(task.getFormData());
    String paramsOssKey = "workers/" + worker.getNodeCode()
                        + "/params/" + task.getTaskCode() + ".json";
    ...
    // 等待 conf-sync 同步后启动进程
    waitForProgramReady(supervisorFactory.getClient(worker), "task-" + execCode, 15, 2);
    supervisorFactory.getClient(worker).startProcess("task-" + execCode);
