...
CREATE TABLE execution_record (
  ...
  mongo_log_ref    VARCHAR(128)  DEFAULT NULL                       COMMENT 'MongoDB 运行日志文档的 execution_code 索引键',
...
任务的运行参数（即用户在创建任务时通过 Formily 表单填写的 `form_data`，存于 MySQL `task.form_data` 字段）需要在执行时注入给爬虫脚本。

**存储路径设计：**

```
OSS: workers/{nodeCode}/params/{executionCode}.json   ← 平台在分发任务时上传
```

**分发时机：**

平台在生成 conf 文件的同时，将 `task.form_data` 序列化为 JSON 并上传到 OSS，两者同步进行：

```java
// TaskDispatchService.dispatch() 中
...
// 并行上传：conf 文件 + params.json
String confOssKey   = "workers/" + worker.getNodeCode() + "/confs/task-" + execCode + ".conf";
String paramsOssKey = "workers/" + worker.getNodeCode() + "/params/" + execCode + ".json";

ossConfDistributor.upload(confOssKey,   confContent);
ossConfDistributor.upload(paramsOssKey, paramsJson);
```

**run_task.sh 中下载 params.json：**

`params.json` 的 OSS 路径通过环境变量 `OSS_PARAMS_KEY` 传入，`run_task.sh` 在下载脚本包后随即下载：
...
EXEC_CODE="${EXECUTION_CODE}"
OSS_KEY="${OSS_SCRIPT_KEY}"
OSS_PARAMS_KEY="${OSS_PARAMS_KEY}"      # 新增
ENTRY_CMD="${ENTRY_COMMAND}"
LANGUAGE="${SCRIPT_LANGUAGE:-python}"
BUCKET="${OSS_BUCKET}"
...
├── params.json          ← 从 OSS 下载（task.form_data）
├── main.py              ← 从脚本包解压
├── requirements.txt
└── utils/
    └── ...
...
**为什么不用 Supervisor environment 变量传递 JSON：**

Supervisor 的 `environment=` 字段要求对值中的特殊字符（双引号、逗号等）进行额外转义，当 `form_data` 包含嵌套 JSON 或中文时极易出错，且 Supervisor 对单个环境变量的长度有限制。存入 OSS 后以文件形式传递，无任何长度和字符限制。


#### 10.3.7 OSS 完整目录结构（含 params）
...
└── workers/
    └── worker-01/
        ├── confs/                         # conf 文件（conf-sync 同步）
        │   ├── task-EXEC-001.conf
        │   └── task-EXEC-002.conf
        └── params/                        # 任务参数（run_task.sh 下载）
            ├── EXEC-001.json              ← task.form_data
            └── EXEC-002.json
```

#### 10.3.8 平台生成的 conf 文件内容
...
environment=EXECUTION_CODE="EXEC-20240318-001",OSS_SCRIPT_KEY="scripts/app-electric/v3.zip",OSS_PARAMS_KEY="workers/worker-01/params/EXEC-20240318-001.json",ENTRY_COMMAND="python main.py --params params.json",SCRIPT_LANGUAGE="python",OSS_BUCKET="crawler-platform"
```

所有执行参数（脚本路径、params 路径、入口命令、语言类型）通过环境变量注入 `run_task.sh`，conf 文件本身无需修改任何 Worker 本地固定配置。
