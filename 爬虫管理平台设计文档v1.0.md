...
environment=OSS_BUCKET="crawler-platform"
...
# NODE_CODE 不再需要，conf 文件统一存储在 oss://bucket/confs/ 下，所有节点共享同步
OSS_BUCKET="${OSS_BUCKET}"
OSS_CONF_PREFIX="confs/"
LOCAL_CONF_DIR="/etc/supervisor/conf.d/tasks/"

mkdir -p "${LOCAL_CONF_DIR}"

while true; do
    # 双向同步：OSS 新增 → 下载到本地；OSS 删除后本地自动清理（--delete）
...
平台在 OSS 上的完整目录规划，涵盖脚本版本、Supervisor conf 分发、任务参数三类文件。**路径中不含 nodeCode**，下发到哪台机器的信息由数据库 `execution_record.worker_node_id` 记录，OSS 只存内容本身。
...
│
├── confs/                                    # Supervisor conf 文件（所有节点共享同步）
│   └── task-{executionCode}.conf             # 平台分发时写入；执行结束后删除
│                                             # conf-sync 每 3s 同步到所有 Worker 本地
│                                             # autostart=false，不会自动执行，仅注册
│
└── params/                                   # 任务运行参数文件（任务级，非节点级）
...
| `confs/task-{executionCode}.conf` | 调度服务 | 分发任务时 | `conf-sync`（所有 Worker） | 每 3s 轮询 |
| `params/{taskCode}.json` | 调度服务 | 分发任务时（参数变更则覆盖） | `run_task.sh` | 脚本执行启动时 |

> conf 文件所有 Worker 都会同步，但 `autostart=false` 保证不会自动运行。平台通过 Supervisor XML-RPC 仅在目标 Worker 上调用 `startProcess`，控制权在平台侧，不依赖节点路径隔离。
...
public ExecutionRecord dispatch(Task task, WorkerNode worker) throws Exception {
    ...
    String confOssKey  = "confs/task-" + execCode + ".conf";
...
public class TaskDispatchService {
    ...
    public ExecutionRecord dispatch(Task task, WorkerNode worker) throws Exception {
        ...
        String ossKey = "confs/task-" + execCode + ".conf";
    ...
    public void stopExecution(String execCode, WorkerNode worker) throws Exception {
        ...
        String ossKey = "confs/" + programName + ".conf";
