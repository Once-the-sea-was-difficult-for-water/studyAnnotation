...
│  └──────────────┘  └──────────────┘  └────────────┘ │
└──────────┬──────────────────┬───────────────────────┘
           │ Supervisor XML-RPC（内网）  │ OSS 读写
           │ 进程控制 / 日志读取         │ conf 文件 / 脚本包
┌──────────▼──────────────────▼───────────────────────┐
│              执行层（多个 Worker 节点）                │
│  Worker-01        Worker-02        Worker-N          │
│  ┌─────────────────────────────────┐                 │
│  │ supervisord（XML-RPC :9001）     │                 │
│  │ conf-sync（OSS → 本地 conf 目录）│                 │
│  │ run_task.sh（OSS → 脚本下载包装）│                 │
│  └─────────────────────────────────┘                 │
└───────────────────────┬─────────────────────────────┘
                        │
┌───────────────────────▼─────────────────────────────┐
│                     存储层                            │
│  MySQL（业务元数据）│ MongoDB（结果/日志）             │
│  OSS（脚本包 + Worker conf 分发文件）                  │
└─────────────────────────────────────────────────────┘
```
...
| **调度服务节点** | 1 | 任务调度、分发、重试、防重叠控制；生成 conf 写入 OSS 并调用 Worker Supervisor 启动进程 |
| **管理服务节点** | 1 | 定时探活 Worker Supervisor（替代心跳）、依赖版本管理、审计日志 |
| **Worker 工作节点** | N（可扩展） | 仅运行 supervisord（XML-RPC 内网开放）和 conf-sync 守护进程，无业务代码；被动接受平台指令 |
...
```
用户                   平台服务（调度节点）              OSS              Worker节点（supervisord）
  │                          │                          │                      │
  │ 在应用广场选择应用          │                          │                      │
  │ ──── 创建任务 ─────────► │                          │                      │
  │     填写参数/调度策略       │ 生成任务记录               │                      │
  ...
  │                          │                          │                      │
  │                          │ ③ 轮询 getProcessInfo()  │                      │
  │ ◄─── 查看执行日志 ─────── │ ④ 轮询 readProcessLog()  │                      │ 持续写 stdout
  │                          │   写入 MongoDB           │                      │
  │                          │                          │                      │ 退出（exitCode）
  │                          │ ⑤ 检测到 EXITED          │                      │
  │                          │   更新执行记录状态          │                      │
...
> **方案选型说明**：采用「Supervisor XML-RPC 内网暴露 + OSS 分发 conf 文件」方案。Worker 节点无需部署任何业务代码（无 agent.py），仅运行 `supervisord` 和一个极简 `conf-sync` 守护进程；进程控制、状态查询、日志读取全部由平台服务通过 Supervisor XML-RPC 远程完成。

#### 10.3.1 Worker 节点组件构成

每个 Worker 节点只安装以下两个组件，无业务逻辑代码：

| 组件 | 形式 | 职责 |
|------|------|------|
| `supervisord` | 系统服务 | 进程拉起与监控，XML-RPC 监听内网 IP:9001 |
| `conf-sync` | Supervisor 托管的 shell 脚本 | 每 3s 从 OSS 同步 conf 文件到本地，触发 supervisorctl update |
| `run_task.sh` | 预装入口包装脚本 | 被 Supervisor 调用，从 OSS 下载脚本包并执行 |
...
; ── 常驻：conf-sync 守护进程 ──
[program:conf-sync]
command=/opt/crawler/conf-sync.sh
autostart=true
autorestart=true
stdout_logfile=/var/log/crawler/conf-sync.log
stderr_logfile=/var/log/crawler/conf-sync_err.log
environment=NODE_CODE="worker-01",OSS_BUCKET="crawler-platform"

; ── 动态任务 conf 通过 include 加载 ──
[include]
files = /etc/supervisor/conf.d/tasks/task-*.conf
```

#### 10.3.3 conf-sync 守护脚本

预装在每个 Worker 节点上（`/opt/crawler/conf-sync.sh`），无业务逻辑，只做文件同步：
...
OSS_CONF_PREFIX="workers/${NODE_CODE}/confs/"
LOCAL_CONF_DIR="/etc/supervisor/conf.d/tasks/"

mkdir -p "${LOCAL_CONF_DIR}"

while true; do
    # 双向同步：OSS 新增 → 下载到本地；本地多余 → 删除（OSS 删除后自动清理）
    ossutil64 sync "oss://${OSS_BUCKET}/${OSS_CONF_PREFIX}" "${LOCAL_CONF_DIR}" \
        --delete --include "task-*.conf" -f -q 2>/dev/null

    # 通知 Supervisor 重新加载配置（有变化时才生效）
...
预装在每个 Worker 节点上（`/opt/crawler/run_task.sh`），从 OSS 下载脚本并执行：
...
OSS_KEY="${OSS_SCRIPT_KEY}"
ENTRY_CMD="${ENTRY_COMMAND}"
LANGUAGE="${SCRIPT_LANGUAGE:-python}"
BUCKET="${OSS_BUCKET}"
...
├── scripts/                          # 爬虫脚本包（已有）
│   └── {appCode}/v{version}.zip
└── workers/                          # Worker 配置分发（新增）
    ├── worker-01/
    │   └── confs/
    │       ├── task-EXEC-001.conf    # 平台写入，conf-sync 下载
    │       └── task-EXEC-002.conf
    └── worker-02/
        └── confs/
            └── task-EXEC-003.conf
...
#### 10.3.6 平台生成的 conf 文件内容

平台为每次执行生成一个 conf 文件，上传到对应 Worker 的 OSS 路径：

```ini
; workers/worker-01/confs/task-EXEC-20240318-001.conf
...
environment=EXECUTION_CODE="EXEC-20240318-001",OSS_SCRIPT_KEY="scripts/app-electric/v3.zip",ENTRY_COMMAND="python main.py --params params.json",SCRIPT_LANGUAGE="python",OSS_BUCKET="crawler-platform"
```

所有执行参数（脚本路径、入口命令、语言类型）通过环境变量注入 `run_task.sh`，conf 文件本身无需修改任何 Worker 本地固定配置。

#### 10.3.7 Java SupervisorClient（远程 XML-RPC）
...
public class TaskDispatchService {
    ...
    private final OssConfDistributor ossConfDistributor;
    ...
    public ExecutionRecord dispatch(Task task, WorkerNode worker) throws Exception {
        ...
        // ① 生成 conf 内容并上传到 OSS
        String confContent = buildConfContent(execCode, task, worker);
        String ossKey = "workers/" + worker.getNodeCode() + "/confs/task-" + execCode + ".conf";
        ossConfDistributor.upload(ossKey, confContent);

        // ② 等待 conf-sync 完成同步（最多等 15s，每 2s 检查一次）
        waitForProgramReady(supervisor, "task-" + execCode, 15, 2);

        // ③ 启动进程
        supervisor.startProcess("task-" + execCode);
    ...
    /** 轮询等待 Supervisor 识别到新 program */
    private void waitForProgramReady(SupervisorClient supervisor,
                                     String programName, int timeoutSec, int intervalSec)
            throws Exception {
        long deadline = System.currentTimeMillis() + timeoutSec * 1000L;
        while (System.currentTimeMillis() < deadline) {
            try {
                supervisor.getProcessInfo(programName);
                return; // program 已存在，可以启动
            } catch (XmlRpcException e) {
                Thread.sleep(intervalSec * 1000L);
            }
        }
        throw new RuntimeException("等待 Supervisor 识别 program 超时：" + programName);
    }
    ...
    public void stopExecution(String execCode, WorkerNode worker) throws Exception {
        ...
        // ③ 删除 OSS 上的 conf 文件（conf-sync 下次同步时自动删除本地文件）
        String ossKey = "workers/" + worker.getNodeCode() + "/confs/" + programName + ".conf";
        ossConfDistributor.delete(ossKey);
...
#### 10.3.9 日志读取（替代 Agent 上报）

不再需要 Worker 主动推送日志，平台通过 Supervisor XML-RPC 主动拉取：
...
public class LogCollector {
    ...
    // 每 500ms 为所有运行中的执行拉取一次新日志
    @Scheduled(fixedDelay = 500)
    public void collectLogs() {
        List<ExecutionRecord> runningList = logService.findRunningExecutions();
        for (ExecutionRecord record : runningList) {
            ...
            try {
                // 从上次读取的偏移量开始读取新增日志（-1 表示追加读取）
                int lastOffset = record.getLogReadOffset();
                String newLog = supervisor.readProcessStdoutLog(programName, lastOffset, 65536);

                if (!newLog.isEmpty()) {
                    logService.appendRaw(record.getId(), newLog);
                    // 更新偏移量，下次从断点继续
                    record.setLogReadOffset(lastOffset + newLog.getBytes(StandardCharsets.UTF_8).length);
                }
            } catch (XmlRpcException e) {
...
#### 10.3.11 Worker 节点注册与 supervisor_port 配置

Worker 节点不再自动注册，由管理员在平台「Worker 节点管理」页面手动添加，填写：

| 字段 | 示例 | 说明 |
|------|------|------|
| 节点编号 | `worker-01` | 唯一标识，与 OSS 路径对应 |
...
#### 10.3.12 多应用多任务并发下的配置全貌

当前有 3 个应用在 Worker-01 上并发运行 4 个任务，OSS 与本地状态如下：

```
OSS: workers/worker-01/confs/
...
Worker-01 本地: /etc/supervisor/conf.d/tasks/
  ├── task-EXEC-001.conf   ← conf-sync 同步
  ├── task-EXEC-002.conf
  ├── task-EXEC-003.conf
  └── task-EXEC-004.conf

supervisorctl status:
  conf-sync                RUNNING   pid 1100, uptime 5 days
...
| 场景 | 处理方式 |
|------|----------|
| conf-sync 同步延迟 | `waitForProgramReady` 最多等 15s，超时则标记执行失败 |
...
| Worker 重启后孤儿 conf 文件 | OSS 上无对应文件 → conf-sync `--delete` 会自动清理本地孤儿 conf |
